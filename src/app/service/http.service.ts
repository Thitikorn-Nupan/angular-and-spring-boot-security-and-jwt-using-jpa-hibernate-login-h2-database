...
export class HttpService {
  ...
  public getProgramingBooks(jwt : string | null) : Observable<Book[]> { // Role USER can access only.
    ...
    return this.httpClient.get<Book[]>(`http://${this.domain}:8082/api/book-store/programing/reads`,{headers});
  }
  public getProgramingBook(jwt : string | null,bid : string | null) : Observable<Book> { // Role USER can access only.
    const headers = new HttpHeaders({ Authorization: 'Bearer ' +jwt} );
    return this.httpClient.get<Book>(`http://${this.domain}:8082/api/book-store/programing/read/${bid}`,{headers});
  }
